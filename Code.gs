...
var manuallyDeletedEventsTrackingDays = 90; // Number of days to remember manually deleted events
...
function startSync(){
  ...
  for (var calendar of sourceCalendars){
    ...
    if(addEventsToCalendar || modifyExistingEvents || removeEventsFromCalendar){
      ...
      // Track manually deleted events

      var manuallyDeletedEvents = trackManuallyDeletedEvents(calendarEventsIds, icsEventsIds);

      Logger.log("Tracking " + Object.keys(manuallyDeletedEvents).length + " manually deleted events");
